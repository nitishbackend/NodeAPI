...
const BatchDetailsModel = require('../models/batch_details');
const BaleDetailsModel = require('../models/bale_details');
	...
	if (password == '') {
		...
		try {
			...
			if (dataFound != null && dataFound != "") {
				...
				return res.status(200).json({ message: "Created sucessfully", userDetails })
			}
...
router.post('/login', async (req, res) => {
	...
	if (password == '') {
		return res.status(200).json({success:false, message: "Password is blank" })
	}

	if (email == '') {
		return res.status(200).json({success:false, message: "Email is blank" })
	}
	...
	try {
		...
		if (dataFound) {
			...
			if (valid) {
				return res.json({success:true, message: "Logged in successfully", token: generateToken(dataFound._id, dataFound.userType, dataFound.email), userId: dataFound.userId });
			...
			return res.json({success:false, message: "Email not registred with us." });
		}
...
//save batch details
router.post('/saveBatchDetails', auth, async (req, res) => {

	try {
		let { userId, farmerLocation, farmerName, harvestdateNtime, dryMatter, crudeProtein, crudeFiber, adf, ndf } = req.body;
		...
		if (!data) {
			return res.status(200).json({success:false, message: "Something went wrong in saving batch details" });
		} else {
			const dataToSave = new BatchDetailsModel({
				userId: data.userId,
				farmerLocation: farmerLocation,
				farmerName: farmerName,
				harvestdateNtime: harvestdateNtime,
				dryMatter: dryMatter,
				crudeProtein: crudeProtein,
				crudeFiber: crudeFiber,
				adf: adf,
				ndf: ndf,
				batchCreationDateNtime: new Date()
			})
			const batchDetails = await dataToSave.save();
			return res.status(200).json({success:true, message: "Batch details saved successfully!", batchDetails:  batchDetails});
		}
...
router.post('/saveBaleDetails', auth, async (req, res) => {

	try {
		let { userId, batchId, weight } = req.body;
		var query = { userId: userId }
		console.log(req.headers['authorization']);
		...
		if (!data) {
			return res.status(200).json({success:false, message: "Something went wrong in saving bale details" });
		} else {
			const dataToSave = new BaleDetailsModel({
				userId: data.userId,
				batchId: batchId,
				weight: weight,
				baleCreationDate: new Date()
			})
			const baleDetails = await dataToSave.save();
			return res.status(200).json({success:true, message: "Bale details saved successfully!", baleDetails:  baleDetails});
		}
